...
> ## SCOPED FOR DUBIZZLE
>
> dubizzle has its own 42-component library and 587 icons. **Check `src/components/`
> and `design-kit/patterns/patterns.css` before reaching for anything here** — a second
> library for something we already have is a defect, not a shortcut.
>
> Where it earns its place is a primitive the system genuinely lacks. The live one today
> is **charting**: the agency portal ships a line chart, the design system has no chart
> tokens and no charting dependency, and `chart-data-viz` says adding one is a decision
> to raise rather than make silently. This skill is how to raise it well.
>
> Any dependency this skill recommends is a proposal: log it, say what it costs in bundle
> size, and get sign-off. Whatever is chosen still renders in dubizzle's tokens.
