...
> place, dismissal and stacking. Adding it is a new dependency, so it is a proposal
> (`docs/PROPOSALS.md`) with its bundle cost stated.
>
> If it is adopted, it is themed entirely in dubizzle tokens and its motion follows
> `RULES.md` 4c and the adopted duration tokens. Never ship Sonner's default look.
