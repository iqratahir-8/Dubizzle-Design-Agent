...
> ## DUBIZZLE TOKENS
>
> This skill's rule 3 already says to extend the codebase's tokens rather than fork
> them. Here they are:
>
> `--ease-standard` `cubic-bezier(0.4, 0, 0.2, 1)` · `--duration-fast` 0.15s ·
> `--duration-base` 0.25s · `--skeleton-duration` 1.5s
>
> They are **adopted, not measured** (D-013): credited to dubizzle's own design-system
> export, not yet verified against live. Use them in preference to this skill's tables
> where they cover the case, and log a genuinely new value in `docs/PROPOSALS.md`.
>
> `RULES.md` 4c also bans, measured or not: `transform: scale()` on hover, bounce /
> spring / elastic easing, staggered entrance reveals, parallax, and anything that
> moves while a listing is being scanned. dubizzle is a scanning product, so this
> skill's own frequency gate ("100+ times/day → no animation, ever") is doubly right
> here.
