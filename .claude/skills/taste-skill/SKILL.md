...
description: >-
  Anti-slop craft for dubizzle Egypt LANDING, CAMPAIGN and MARKETING pages — the surfaces
  where composition is a real choice. Use for a campaign page, a promo or seasonal landing
  page, a growth or SEO page, a feature announcement, or an app-download page. Do NOT use it
  for product UI: listings, search, ad detail, post-an-ad, chat, account or the agency portal
  are measured surfaces and belong to the `feature-design` skill. Brand values stay binding
  either way.
---

> ## SCOPED FOR DUBIZZLE — read this before the skill below
>
> This is the upstream `taste-skill` (leonxlnx), installed for **marketing surfaces only**.
> Its own opening line is the reason: *"Landing pages, portfolios, and redesigns. Not
> dashboards, not data tables, not multi-step product UI."* dubizzle's product UI is a
> measured surface — 142 live captures, 1,269 tokens — and `feature-design` owns it.
>
> On a dubizzle landing page, this skill governs **composition**: hierarchy, rhythm,
> section order, restraint, how not to look templated. It does **not** get to choose the
> ingredients. These stay binding and outrank anything below:
>
> | Binding | Where |
> |---|---|
...
> | EGP, real Egyptian locations, real listing content | `design-kit/content/fixtures.json` |
> | Contrast — 14 of 24 palette pairings fail AA; check before shipping | `RULES.md` §4b, `npm run check:a11y` |
> | Arabic/RTL — a landing page must work in both | `rtl-arabic` skill |
>
> **What this skill unlocks that product UI forbids:** a centred hero with one CTA, three
...
> state a design read, refuse the templated defaults, sweat the hierarchy.
>
> Anything genuinely new still goes to `docs/PROPOSALS.md` and gets said out loud. Finish with
> `npm run check:design` and `npm run check:a11y`.
>
> — end of dubizzle scoping; upstream skill follows —
