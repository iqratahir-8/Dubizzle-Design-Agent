...
> ## SCOPED FOR DUBIZZLE — read before applying any of this
>
> Installed deliberately and **ahead of the rules**, for the day dubizzle moves its
> components and physics toward a newer iOS feel (user decision, 2026-09-18).
>
> **As of today, `RULES.md` 4c forbids what this skill teaches**: spring, bounce and
> elastic easing are banned, along with parallax and anything that moves while a list
> is being scanned. That is not an oversight — dubizzle is a high-volume scanning
> product, and the ban predates this skill.
>
> So the rule is simple: **this skill produces proposals, not shipped motion.**
>
> | Surface | What this skill may do |
> |---|---|
> | Measured (listings, search, DPV, post-an-ad, chat, account, portal) | Nothing ships. Write the spring you would use into `docs/PROPOSALS.md` with the interaction it serves, and say it needs sign-off **and a change to RULES.md 4c**. |
> | Composed (landing, campaign) | More room, but still: any spring is a new motion value, so it is a proposal. |
> | Prototypes and explorations | Free rein. Label the output a prototype. |
>
> Where it genuinely applies today, unchanged: **interruptibility** (motion should be
> grabbable and reversible mid-flight), **starting from the current on-screen value**
> rather than from zero, and **respecting the user's velocity** on a drag. Those are
> correctness, not style, and none of them require a spring.
>
> Always pair with `prefers-reduced-motion`, which no dubizzle component honours yet.
>
> Changing 4c is a designer and product decision (see `PRODUCT.md`), not one this skill
> or Claude makes. Until it changes, a spring in a measured surface is a defect.
