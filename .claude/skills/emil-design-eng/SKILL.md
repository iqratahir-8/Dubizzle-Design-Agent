...
> ## SCOPED FOR DUBIZZLE
>
> Emil Kowalski's design-engineering philosophy (MIT), installed for **craft**: the
> invisible details, interaction quality, and why an interface feels right.
>
> It does **not** supply values here. dubizzle's palette, type, spacing, radius,
> shadow and motion are measured from production across 142 captures, and
> `RULES.md` outranks anything below. Where this skill suggests a border, a shadow,
> a radius or a duration that differs from a token, **the token wins** and the
> suggestion goes to `docs/PROPOSALS.md` if it is worth raising.
>
> Its "taste is trained" framing is about judgement, not licence to pick new
> ingredients. Read D-007 and D-014 before treating any recommendation here as a
> value to adopt.
>
> On **measured surfaces** (listings, search, DPV, post-an-ad, chat, account,
> portal) this skill advises on interaction quality only. On **composed surfaces**
> (landing, campaign) it has more room, alongside `design-taste-frontend`.
